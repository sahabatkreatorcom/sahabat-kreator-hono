...
platformWebhookRoute.get("/webhooks/meta", (c) => {
  const challenge = metaVerification(c, env.META_WEBHOOK_VERIFY_TOKEN);
...
platformWebhookRoute.get("/webhooks/instagram-standalone", (c) => {
  const challenge = metaVerification(
    c,
    env.INSTAGRAM_WEBHOOK_VERIFY_TOKEN ?? env.META_WEBHOOK_VERIFY_TOKEN,
  );
  if (challenge === null) return c.text("Forbidden", 403);
  return c.text(challenge);
});
...
platformWebhookRoute.get("/webhooks/threads", (c) => {
  const challenge = metaVerification(
    c,
